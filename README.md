...
### 摘要
```
使用机器学习打分法及深度学习端到端学习两种方案解决该问题.
1. 机器学习打分法: 将任务看作回归任务, 建立回归预测模型再rank得到性能排序, 回归模型训练前将target转换为标准正态分布;
2. 深度学习端到端法: 将不可导的rank相关性kendall设计成可导的soft形式, 使用 1 - soft_kendall 作为loss直接训练, 模型结构使用Bi-LSTM及transformer encoder, decoder;
